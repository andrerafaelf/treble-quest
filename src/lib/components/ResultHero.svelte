<script lang="ts">
  ...
  const nextChallenge = $derived((): string => {
    if (worldCup) {
      ...
      if (worldCup.won) return `${8 - worldCup.wins} result${8 - worldCup.wins === 1 ? '' : 's'} from a perfect 8-0. Go again.`;
    ...
    if (result.trophies === 3) return `Treble done with ${result.league.points} pts. Chase 38-0 and a perfect European run.`;
